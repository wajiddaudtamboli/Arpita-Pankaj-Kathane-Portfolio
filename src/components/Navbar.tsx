    ...
    <>
      <motion.nav
        ...
        className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
          scrolled
            ? 'bg-background/90 backdrop-blur-xl border-b border-border shadow-card-premium'
            : 'bg-transparent'
        }`}
      >
        <div className="container-max grid grid-cols-[auto_1fr_auto] items-center gap-4 h-16 px-4 sm:px-6 lg:px-8">
          ...
          {/* Desktop Nav Links */}
          <div className="hidden xl:grid grid-cols-9 gap-2 w-full max-w-5xl mx-auto px-4">
            {navLinks.map(link => (
              <a
                key={link.key}
                href={link.href}
                onClick={e => { e.preventDefault(); handleNavClick(link.href); }}
                className="h-9 px-2 flex items-center justify-center text-center text-[11px] font-semibold text-muted-foreground hover:text-foreground rounded-md transition-colors hover:bg-muted border border-transparent hover:border-border"
              >
          ...
          <div className="flex items-center gap-2 justify-self-end">
            ...
            <div className="relative">
              <button
                ...
                className="flex items-center gap-1 px-2.5 py-1.5 rounded-md text-xs font-semibold text-muted-foreground hover:text-foreground hover:bg-muted transition-colors border border-border"
              >
              ...
              <AnimatePresence>
                {langOpen && (
                  <motion.div
                    ...
                    className="absolute right-0 top-full mt-1 w-28 bg-card border border-border rounded-lg shadow-lg overflow-hidden"
